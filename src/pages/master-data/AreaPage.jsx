  ...
  return (
    <>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-black text-[#052334]">Master Data Area</h1>
          <p className="text-slate-500 text-sm mt-1">Kelola daftar lokasi dan area infrastruktur perusahaan.</p>
        </div>
        <button onClick={() => setIsModalOpen(true)} className="bg-[#052334] hover:bg-[#0A3349] text-white px-5 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 shadow-md shadow-[#052334]/10 transition-colors">
          <Plus className="w-4 h-4" /> Tambah Data Area
        </button>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100">
            {paginatedData.length > 0 ? (
              ...
              <tr>
                <td colSpan="3" className="px-8 py-8 text-center text-sm text-slate-400 font-medium">Data tidak ditemukan</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center px-6 py-5 border-b border-slate-100">
              <h3 className="text-base font-bold text-slate-800">Tambah Data Area</h3>
            ...
            <form onSubmit={async (e) => {
              e.preventDefault();
              if (!newDataName.trim()) { 
                Swal.fire({ title: 'Perhatian', text: 'Nama area tidak boleh kosong!', icon: 'warning', confirmButtonColor: '#E58032' }); 
                return; 
              }
              try {
                await api.post('/areas', { name: newDataName });
                fetchAreas();
                setIsModalOpen(false);
                setNewDataName('');
                Swal.fire({ title: 'Berhasil!', text: 'Area berhasil ditambahkan!', icon: 'success', confirmButtonColor: '#052334', toast: true, position: 'top-end', timer: 3000, showConfirmButton: false });
              } catch (error) {
              ...
              <div className="p-6">
                <label className="block text-[11px] font-bold text-slate-500 mb-2 uppercase tracking-wide">Nama Area</label>
                <input 
                  type="text" 
                  value={newDataName}
