...
const JenisBangunanPage = () => {
  ...
  const fetchJenisBangunan = async () => {
    try {
      const res = await api.get('/jenis-bangunan');
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-black text-[#052334]">Master Data Jenis Bangunan</h1>
          <p className="text-slate-500 text-sm mt-1">Kelola klasifikasi dan jenis konstruksi bangunan.</p>
        </div>
        <button onClick={() => setIsModalOpen(true)} className="bg-[#052334] hover:bg-[#0A3349] text-white px-5 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 shadow-md shadow-[#052334]/10 transition-colors">
          <Plus className="w-4 h-4" /> Tambah Jenis Bangunan
        </button>
      </div>
      ...
      <div className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.02)] mb-6">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="relative w-full md:w-72">
            ...
            <input 
              type="text" 
              placeholder="Cari jenis bangunan..." 
        ...
        <table className="w-full text-left border-collapse">
          <thead className="bg-slate-50/80 text-slate-500 text-[11px] uppercase tracking-wider border-b border-slate-200/80">
            <tr>
              ...
              <th className="px-8 py-4 font-bold">Jenis Bangunan</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center px-6 py-5 border-b border-slate-100">
              <h3 className="text-base font-bold text-slate-800">Tambah Jenis Bangunan</h3>
            ...
            <form onSubmit={async (e) => {
              e.preventDefault();
              if (!newDataName.trim()) { 
                Swal.fire({ title: 'Perhatian', text: 'Nama jenis bangunan tidak boleh kosong!', icon: 'warning', confirmButtonColor: '#E58032' }); 
                return; 
              }
              try {
                await api.post('/jenis-bangunan', { name: newDataName });
                fetchJenisBangunan();
                setIsModalOpen(false);
                setNewDataName('');
                Swal.fire({ title: 'Berhasil!', text: 'Jenis bangunan berhasil ditambahkan!', icon: 'success', confirmButtonColor: '#052334', toast: true, position: 'top-end', timer: 3000, showConfirmButton: false });
              } catch (error) {
...
export default JenisBangunanPage;