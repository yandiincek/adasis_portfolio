    ...
    <div className="max-w-7xl mx-auto pb-12">
      <div className="mb-8">
        <h1 className="text-2xl font-black text-[#052334] flex items-center gap-3">
          ...
          Report PA Before Backup
        </h1>
        <p className="text-slate-500 text-sm mt-1 mb-6">
          Persentase Ketersediaan Fisik (Physical Availability) sebelum unit pengganti diturunkan.
        </p>
        ...
        {summaryData && (
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm mb-6 overflow-hidden">
            <div className="bg-slate-50 py-3 px-4 border-b border-slate-200 flex items-center gap-2">
              ...
              <h2 className="text-[#052334] font-black text-sm uppercase tracking-wider">KPI Summary By Type {filterArea ? `(${filterArea})` : ''}</h2>
            </div>
          ...
          <table className="w-full text-left border-collapse min-w-max">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="px-4 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider text-center">Jam Breakdown (Real)</th>
                <th className="px-4 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider text-center">PA Before Backup</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                ...
                filteredData.map((item, index) => {
                  // Warna bar progress
                  const paVal = parseFloat(item.pa_before_persen);
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                      ...
                      <td className="px-4 py-3 text-sm text-center font-medium text-red-500">{item.jam_bd_real} Jam</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3 justify-center w-full">
                          <span className="text-sm font-bold w-12 text-right">{item.pa_before_persen}%</span>
                ...
                <tr>
                  <td colSpan="10" className="px-4 py-8 text-center text-slate-500">Tidak ada data sarana.</td>
                </tr>
              )}
            </tbody>
          </table>
...
export default ReportPABeforePage;
